...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--convert-3d', action='store_true', help='convert 3D mocap data'
    )
    parser.add_argument(
        '--convert-2d',
        default='',
    ...
    if args.path:
        ...
        for subject in subjects:
            ...
            for f in file_list:
                ...
                positions_2d = hf['poses_2d'].transpose(
                    1, 0, 2, 3
                )  # Ground-truth 2D poses
                assert (
                    positions.shape[0] == positions_2d.shape[0]
                    and positions.shape[1] == positions_2d.shape[2]
                )
                assert (
                    num_joints is None or num_joints == positions.shape[1]
                ), "Joint number inconsistency among files"
                num_joints = positions.shape[1]

                # Sanity check for the sequence length
                assert (
                    positions.shape[0]
                    == index[subject][canonical_name][1]
                    - index[subject][canonical_name][0]
                )
                ...
                all_chunks = [
                    list(v)
                    for k, v in groupby(positions, lambda x: np.isfinite(x).all())
                ]
                all_chunks_2d = [
                    list(v)
                    for k, v in groupby(positions_2d, lambda x: np.isfinite(x).all())
                ]
                ...
                for i, chunk in enumerate(all_chunks):
                    ...
                    if np.isfinite(chunk).all():
                        ...
                        output_2d[subject][name] = list(
                            np.array(all_chunks_2d[i], dtype='float32').transpose(
                                1, 0, 2, 3
                            )
                        )
        ...
        if args.convert_3d:
            ...
            np.savez_compressed(
                output_prefix_2d + 'gt', positions_2d=output_2d, metadata=metadata
            )
            print('Done.')
    ...
    if args.convert_2d:
        ...
        for f in file_list:
            ...
            assert subject.startswith('S'), (
                subject + ' does not look like a subject directory'
            )
            ...
            if subject in frame_mapping and action in frame_mapping[subject]:
                ...
                for (start_idx, end_idx, labeled, split, name) in chunks:
                    ...
                    assert (
                        len(kps) == end_idx - start_idx
                    ), "Got len {}, expected {}".format(len(kps), end_idx - start_idx)
                ...
                output[canonical_subject][action][camera_idx] = keypoints.astype(
                    'float32'
                )
