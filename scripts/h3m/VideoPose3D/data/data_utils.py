...
humaneva15_metadata = {
    ...
    'num_joints': 15,
    'keypoints_symmetry': [[2, 3, 4, 8, 9, 10], [5, 6, 7, 11, 12, 13]],
}

humaneva20_metadata = {
    'layout_name': 'humaneva20',
    'num_joints': 20,
    'keypoints_symmetry': [[3, 4, 5, 6, 11, 12, 13, 14], [7, 8, 9, 10, 15, 16, 17, 18]],
}
...
def suggest_metadata(name):
    ...
    raise KeyError(
        'Cannot infer keypoint layout from name "{}". Tried {}.'.format(name, names)
    )
...
def suggest_pose_importer(name):
    ...
    raise KeyError(
        'Cannot infer keypoint format from name "{}". Tried detectron, cpn, sh.'.format(
            name
        )
    )
