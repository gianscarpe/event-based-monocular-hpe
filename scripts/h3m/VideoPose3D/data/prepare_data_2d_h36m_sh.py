...
metadata = {
    'num_joints': 16,
    'keypoints_symmetry': [[3, 4, 5, 13, 14, 15], [0, 1, 2, 10, 11, 12],],
}


def process_subject(subject, file_list, output):
    if subject == 'S11':
        assert len(file_list) == 119, (
            "Expected 119 files for subject " + subject + ", got " + str(len(file_list))
        )
    else:
        assert len(file_list) == 120, (
            "Expected 120 files for subject " + subject + ", got " + str(len(file_list))
        )

    for f in file_list:
        action, cam = (
            os.path.splitext(os.path.basename(f))[0].replace('_', ' ').split('.')
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        metavar='PATH',
        help='convert pretrained dataset',
    )
    parser.add_argument(
        '-ft',
        '--fine-tuned',
        ...
        type=str,
        metavar='PATH',
        help='convert fine-tuned dataset',
    )
