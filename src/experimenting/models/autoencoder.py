...
class AutoEncoder(nn.Module):
    def __init__(self, in_channels, in_cnn, mid_dimension,
                 latent_size, up_layers=4):
        ...
        self.encoder_head = nn.Linear(self.mid_feature_dimension *
        ...
        self.d_fc_1 = nn.Linear(latent_size, self.dec_channels * (2**up_layers)
                                * self.mid_tensor_dimension *
                                self.mid_tensor_dimension) 
        ...
        for i in reversed(range(2, up_layers+1)):
            ...
            stage = nn.Sequential(nn.Conv2d(in_channels, out_channels,
                                            kernel_size=(4, 4), stride=(1, 1),
                                            padding=0), nn.LeakyReLU(0.2,
                                                                     inplace=True),
    ...
    def encode(self, x):
        ...
        x = x.view(-1, self.mid_feature_dimension * self.mid_tensor_dimension *
                   self.mid_tensor_dimension)
