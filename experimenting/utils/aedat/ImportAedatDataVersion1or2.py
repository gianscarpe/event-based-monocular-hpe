...
def import_aedat_dataversion1or2(info):
    ...
    num_events_in_file = int(
        np.floor((file_handle.tell() - info['beginningOfDataPointer']) /
                 num_bytes_per_event))
    ...
    # Read events
    file_handle.seek(info['beginningOfDataPointer'] +
                     num_bytes_per_event * info['startEvent'])
    ...
    polarity_logical = np.logical_and(
        np.logical_not(aps_or_imu_logical),
    ...
    if ('dataTypes' not in info or 'polarity' in info['dataTypes']) \
        ...
        output['data']['polarity']['cam'] = np.array(
            np.right_shift(
                ...
                camera_shift_bits), 'int32')
        print(output['data']['polarity']['cam'])
        output['data']['polarity']['timeStamp'] = all_ts[polarity_logical]
        # Y addresses
        output['data']['polarity']['y'] = np.array(
            np.right_shift(np.bitwise_and(all_addr[polarity_logical], y_mask),
                           y_shift_bits), 'int32')
        # X addresses
        output['data']['polarity']['x'] = np.array(
            np.right_shift(np.bitwise_and(all_addr[polarity_logical], x_mask),
                           x_shift_bits), 'int32')
        # Polarity bit
