...
class BaseConstructor(ABC):
    def __init__(self, hparams, dataset_task):
        ...
        preprocess_val, self.val_aug_info = get_augmentation(hparams.augmentation_test)
    ...
    def get_datasets(self):
        return (
            self.dataset_task(**self.train_params),
            self.dataset_task(**self.val_params),
            self.dataset_task(**self.test_params),
        )
...
class AutoEncoderConstructor(BaseConstructor):
    def __init__(self, hparams):
        super(AutoEncoderConstructor, self).__init__(hparams, AutoEncoderDataset)
